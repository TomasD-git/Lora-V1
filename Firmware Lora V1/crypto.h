#pragma once
#include <stdint.h>
#include <string.h>

static const uint8_t DEFAULT_CHANNEL_KEY[16] = {
  0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
};

static const uint8_t SBOX[256] = {
  
};

static const uint8_t RCON[11] = {
  0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

class AES128 {
public:
  void setKey(const uint8_t* key) {
    memcpy(_roundKey, key, 16);
    keyExpansion();
  }
  void encryptBlock(const uint8_t* in, uint8_t* out) {
    uint8_t state[16];
    memcpy(state, in, 16);

    addRoundKey(state, 0);
    for (int round = 1; round < 10; round++) {
      subBytes(state);
      shiftRows(state);
      mixColumns(state);
      addRoundKey(state, round);
    }
    subBytes(state);
    shiftRows(state);
    addRoundKey(state, 10);
    memcpy(out, state, 16);
  }

private:
  uint8_t _roundKey[176]; 

  void keyExpansion() {
    for (int i = 16; i < 176; i += 4) {
      uint8_t temp[4];
      memcpy(temp, _roundKey + i - 4, 4);
      if ((i % 16) == 0) {
        uint8_t t = temp[0];
        temp[0] = SBOX[temp[1]] ^ RCON[i/16];
        temp[1] = SBOX[temp[2]];
        temp[2] = SBOX[temp[3]];
        temp[3] = SBOX[t];
      }
      for (int j = 0; j < 4; j++)
        _roundKey[i+j] = _roundKey[i-16+j] ^ temp[j];
    }
  }

  void addRoundKey(uint8_t* state, int round) {
    for (int i = 0; i < 16; i++)
      state[i] ^= _roundKey[round*16 + i];
  }

  void subBytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) state[i] = SBOX[state[i]];
  }

  void shiftRows(uint8_t* state) {
    uint8_t tmp;
    tmp=state[1]; state[1]=state[5]; state[5]=state[9]; state[9]=state[13]; state[13]=tmp;
    tmp=state[2]; state[2]=state[10]; state[10]=tmp;
    tmp=state[6]; state[6]=state[14]; state[14]=tmp;
    tmp=state[15]; state[15]=state[11]; state[11]=state[7]; state[7]=state[3]; state[3]=tmp;
  }

  static uint8_t xtime(uint8_t x) { return (x<<1) ^ ((x>>7) ? 0x1b : 0); }

  void mixColumns(uint8_t* state) {
    for (int i = 0; i < 4; i++) {
      uint8_t* s = state + i*4;
      uint8_t a=s[0],b=s[1],c=s[2],d=s[3];
      s[0]=xtime(a)^xtime(b)^b^c^d;
      s[1]=a^xtime(b)^xtime(c)^c^d;
      s[2]=a^b^xtime(c)^xtime(d)^d;
      s[3]=xtime(a)^a^b^c^xtime(d);
    }
  }
};

inline void aesCTR(uint8_t* data, size_t len,
                   const uint8_t* key, const uint8_t* nonce8) {
  AES128 aes;
  aes.setKey(key);

  uint8_t counter[16] = {0};
  memcpy(counter, nonce8, 8); 

  uint8_t keystream[16];
  for (size_t i = 0; i < len; i++) {
    if ((i & 15) == 0) {
      aes.encryptBlock(counter, keystream);
      for (int j = 8; j < 16; j++) {
        if (++counter[j]) break;
      }
    }
    data[i] ^= keystream[i & 15];
  }
}
